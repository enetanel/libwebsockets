...
enum lws_urldecode_fileupload_states {
	LWS_UFS_CONTENT,
	LWS_UFS_FINAL_CONTENT,
	LWS_UFS_OPEN
};

typedef int (*lws_urldecode_fileupload_cb)(void *data, const char *name,
			const char *filename, char *buf, int len,
			enum lws_urldecode_fileupload_states state);

struct lws_urldecode_stateful_param_array;

LWS_VISIBLE LWS_EXTERN struct lws_urldecode_stateful_param_array *
lws_urldecode_spa_create(struct lws *wsi, const char * const *param_names,
			 int count_params,
			 int max_storage, lws_urldecode_fileupload_cb opt_cb,
			 void *opt_data);
